...
import { ValidationRuleType } from 'devextreme-angular/common';
...
export class DevDataGridService {
  ...
  getObjectField(id: number): Observable<Fields[]>{
  ...
  getObjectById(id: number): Observable<Objects> {
    return this.http.get<Objects>(environment.baseApi+"objects/"+id);
  }

  getTableDataByObjectName(objectName: string):any{
    return this.http.get<any[]>(environment.baseApi+objectName);
  }
  updateTableDataByObjectName(objectName: string,key: number, updatedData:any):any {
    return this.http.put<any>(environment.baseApi+objectName+"/"+key,updatedData);
  }
  addTableDataByObjectName(objectName: string, addData:any):any {
    return this.http.post<any>(environment.baseApi+objectName,addData);
  }
  deleteTableDataByObjectName(objectName: string, key:number):any {
    return this.http.delete<any>(environment.baseApi+objectName+"/"+key);
  }
...
const settings: any= {
  ...
  sortingMode:"multiple", //multiple,single
...
export interface Fields {
  ...
  displayName:string,
  width:string,
  dataType:'string' | 'number' | 'date' | 'boolean' | 'object' | 'datetime' | null,
  format:string,
  isVisible:boolean,
  editorType:FormItemComponent | null,
